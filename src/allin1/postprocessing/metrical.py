...
def postprocess_metrical_structure(
  ...
  raw_prob_beats = torch.sigmoid(logits.logits_beat[0]).detach()
  raw_prob_downbeats = torch.sigmoid(logits.logits_downbeat[0]).detach()
  ...
  activations_combined = activations_combined.detach().cpu().numpy()
